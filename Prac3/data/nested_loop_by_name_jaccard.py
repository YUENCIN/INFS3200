...
def nested_loop_by_name_jaccard(threshold, q):
    '''
    ...
    r = measure.calc_measure(results)
    return r
...
for q in [0, 1, 2, 3, 4]:
    value_by_thesh = []
    for threshold in [0, 0.375, 0.5, 0.625, 0.75, 0.875]:
        batch = []
        print("Threshold =", threshold, "Q =", q)
        r = nested_loop_by_name_jaccard(threshold, q)
        value_by_thesh.append(r)
    value.append(value_by_thesh)
